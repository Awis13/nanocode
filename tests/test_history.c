...
TEST(test_history_roundtrip_5turns)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        /* Content may differ due to escaping — just check it is non-empty. */
        ASSERT_TRUE(strlen(conv->turns[i].content) > 0);
    }
...
TEST(test_history_open_and_append)
{
    ...
    history_append_turn(hx, "user",      "first turn",  5);
    history_append_turn(hx, "assistant", "second turn", 10);
    history_append_turn(hx, "user",      "third turn",  3);
...
{
    /* Should not crash. */
    history_append_turn(NULL, "user", "content", 0);
    history_close(NULL);
}
